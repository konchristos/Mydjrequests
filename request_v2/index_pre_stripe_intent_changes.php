...
if (e.target.classList.contains("highlight-btn")) {
    ...
    (async () => {
        try {
            ...
            const res = await fetch(
                "/api/public/create_boost_checkout.php",
                {
                    method: "POST",
                    body: fd
                }
...
<script>
document.addEventListener("DOMContentLoaded", () => {
    const tipBtn = document.getElementById("tipDjBtn");

    if (!tipBtn) {
        console.warn("Tip button not found");
        return;
    }
    ...
    tipBtn.addEventListener("click", async (e) => {
        ...
        try {
            const fd = new FormData();
            fd.append("event_uuid", "<?= e($uuid); ?>");
            fd.append("amount", 500); // $5 default (cents)

            const res = await fetch(
                "/api/public/create_tip_checkout.php",
                {
                    method: "POST",
                    body: fd
                }
...
document.querySelectorAll(".tip-preset").forEach(btn => {
    btn.addEventListener("click", async () => {
        ...
        try {
            ...
            const res = await fetch("/api/public/create_tip_checkout.php", {
                method: "POST",
                body: fd
            });
