...
document.addEventListener("DOMContentLoaded", () => {
 ...
 async function sendMood(mood) {
    ...
    async function fetchMoodStats() {
        try {
            const res = await fetch('/api/mood_stats.php?event=' + encodeURIComponent(eventUuid));
            const data = await res.json();
            if (!data.ok) return;
            ...
            if (total === 0 || score === null) {
                ...
                moodScoreEl.innerHTML =
                    "Crowd mood: <strong>" + score + "% positive</strong><br>" +
                    "👍 " + positive + " / 👎 " + negative;
                ...
                // Multi-color bar
                if (score >= 70) {
                    moodBarInner.style.backgroundColor = "#2cc84b"; // green
                } else if (score >= 40) {
                    moodBarInner.style.backgroundColor = "#f1c40f"; // yellow
                } else {
                    moodBarInner.style.backgroundColor = "#e74c3c"; // red
                }
            }
    ...
    btnPos.onclick = () => sendMood(1);
    btnNeg.onclick = () => sendMood(-1);

    fetchMoodStats();
    setInterval(fetchMoodStats, 30000);
});
</script>